...
public:
  ...
  void SetWinRect(const RECT&rect){TQuad&r=*(TQuad*)(void*)(&rect);Set(r.x,r.y,r.w-r.x,r.h-r.y);}
  void SetSize(const SIZE&size){w=size.cx;h=size.cy;}
  static TQuad getFullScreen(){TQuad tmp;auto mode=GetScreenMode();tmp.Set(0,0,mode.W,mode.H);return std::move(tmp);}
  ...
  void setAsFullScreen(){*this=getFullScreen();}
...
public:
  ...
  static void KeyboardUpdate(QapKeyboard&Keyboard,const HWND hWnd,const UINT msg,const WPARAM wParam,const LPARAM lParam)
  {
    ...
    #undef F
    //---
    if(msg==WM_KEYUP){Keyboard.KeyUpdate(wParam,false);}
    if(msg==WM_KEYDOWN){Keyboard.KeyUpdate(wParam,true);}
    if(msg==WM_LBUTTONUP){Keyboard.KeyUpdate(mbLeft,false);}
    if(msg==WM_RBUTTONUP){Keyboard.KeyUpdate(mbRight,false);}
    if(msg==WM_MBUTTONUP){Keyboard.KeyUpdate(mbMiddle,false);}
    if(msg==WM_LBUTTONDOWN){Keyboard.KeyUpdate(mbLeft,true);}
    if(msg==WM_RBUTTONDOWN){Keyboard.KeyUpdate(mbRight,true);}
...
class T20140604;class QapTreeDebugger;
...
public:
  ...
  static string get_file_name(){return "data.qap";}
...
public:
  virtual void GetSize(IRenderProgram::t_context&context,vec2i&size){size=vec2i_zero;}
public:
...
public:
  static bool toMonoEx(const string&line,string&mono)
  {
    bool flag=false;
    bool good=false;
    bool need=false;mono.reserve(line.size());
...
public:
  ...
  public:
    ...
    struct TMeCallBack:OutDevice::IExternalCallBack
    {
      ...
      TBranch&Branch;
      QapKeyboard&KB;
      TMeCallBack(TOutDevEx::TCmdHeadVisitor&Head,TBranch&Branch,QapKeyboard&KB):Head(Head),Branch(Branch),KB(KB){index=-1;}
...
public:
  ...
  public:
    void OnVar(OutDevice&OD,LineField&Field){if(!KB.OnDown(VK_CONTROL))return;BranchCode(OD);}
    void OnType(OutDevice&OD,LineField&Field){if(!KB.OnDown(VK_CONTROL))return;MicroCode(OD);}
    void OnValue(OutDevice&OD,LineField&Field)
    {
      ...
      if("int"==*Field.pType)
      {
        ...
        for(;;)
        {
          if(KB.OnDown(VK_BACK)){value/=10;}
          if(KB.News)if(KB.LastChar=='-'){value*=-1;KB.News=false;}
      ...
      DownC=false;DownLC=false;DownRC=false;DownDel=false;DownDiv=false;
    }
...
class TDay20140605BookV00; class TDay20140605Book;
...
public:
  struct t_context{
    ...
    vec2i ScreenSpaceToPageSpace(const vec2i&p){return p-pos;}
    vec2i PageSpaceToScreenSpace(const vec2i&p){return p+pos;}
  };
  virtual void Render(t_context&con){}
  virtual void Update(t_context&con){}
};
...
public:
  ...
  void DoSelectPage(t_context&con)
  {
    ...
    for(int i=0;i<arr.size();i++)
    {
      ...
      if(ok)
      {
        ...
        if(need_remove){ex->NeedRemove=true;}
        return;
      }
      //
      curpos.x+=s.x+border_size;
  ...
  void InitPages(IEnvRTTI&Env)
  {
    ...
    {auto*p=AddItem<TPageWithPageAdder>(Env);p->Caption=" + ";}
    {auto*p=AddItem<TBookPageWithQuit>(Env);p->Caption=" X ";}
    {auto*p=AddItem<TBookPageWithDisable>(Env);p->Caption=" _ ";}
  }
  void DoClean()
  {
...
public:
  ...
  void Update(t_context&con)
  {
    ...
    if(script=="y")
    {
      ...
      auto&a=books[0].rect;
      auto&b=books[1].rect;
      int bs=2;int cx=con.size.x;int cy=con.size.y;
      a.pos.y=-cy/4-bs/2;
      b.pos.y=+cy/4+bs/2;
      a.h=(-4*bs+cy)/2;
      b.h=(-4*bs+cy)/2;
...
public:
  ...
  void Update(t_context&con)
  {
    ...
    TOutDevScopeMemberID devscope(con.context.dev,SelfClass::ProxySys$$::GET_FIELD_ID::books);
    books.Update(con);
  }
};
