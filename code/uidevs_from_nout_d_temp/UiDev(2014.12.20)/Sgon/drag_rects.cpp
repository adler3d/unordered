...
class TBookPageWithDragRects;

struct t_super_context{
  IEnvRTTI&Env;
  TD3DGameBoxBuilder&root;
  TBookPageWithDragRects&head;
  TSuperOutDevice dev;
};
...
class TBookPageWithDragRects:public IBookPage{
public:
#define DEF_PRO_STRUCT_INFO(NAME,PARENT,OWNER)NAME(TBookPageWithDragRects)PARENT(IBookPage)
#define DEF_PRO_VARIABLE(ADDBEG,ADDVAR,ADDEND)\
ADDBEG()\
...
namespace
{
  struct ThisCompileUnit:public ICompileUnit
  {
    ...
    virtual void RegAll(IEnvRTTI&Env)
    {
      ...
      #define LIST(F)\
        F(TType)F(TBookPageWithDragRects)F(TUniqueContentWithString)
