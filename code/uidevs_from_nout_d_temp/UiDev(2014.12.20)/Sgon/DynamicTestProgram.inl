...
public:
  ...
  public:
    void DoVisitSaveID(int id,bool safe=false,TType*elemType=nullptr)
    {
      ...
      auto&item=Space[id];
      bool good=item.ptr_lnk.IsValid();
      if(!good)SkipCount++;
      return;
    }
  public:
...
public:
  ...
  static void InitSubEnv(IEnvRTTI&Env,DeclareEnvRTTI&Source,IEnvRTTI&Target)
  {
    ...
    auto&ExpectedInBeforeLoadTime=Machine.Tools.DeclEnv.Arr;//заместители известных на этапе перд-загрузки типов.
    LinkingAndTesting(Machine.Tools,SpaceAdv,ExpectedInBeforeLoadTime,ExpectedInLoadTime);
  }
  void Work(IEnvRTTI&Env,DeclareEnvRTTI&Source,IEnvRTTI&Target)
  {
    InitSubEnv(Env,Source,Target);
  }
