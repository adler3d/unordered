...
public:
  TRect(){}
  TRect(const TRect&ref){a=ref.a;b=ref.b;}
...
inline vector<vec2i> MakeQuadAABB(const vec2i&Min,const vec2i&Max){
  vector<vec2i> PA;PA.resize(4);
  PA[0]=vec2i(Min.x,Min.y);PA[1]=vec2i(Min.x,Max.y);
  PA[2]=vec2i(Max.x,Max.y);PA[3]=vec2i(Max.x,Min.y);
  return PA;
}
