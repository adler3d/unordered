...
class T_2018_BookPage:public IBookPage{
public:
#define DEF_PRO_STRUCT_INFO(NAME,PARENT,OWNER)NAME(T_2018_BookPage)PARENT(IBookPage)
#define DEF_PRO_VARIABLE(ADDBEG,ADDVAR,ADDEND)\
ADDBEG()\
ADDVAR(string,comment,SET,"need function that find object by id in tree",$)\
ADDEND()
//=====+>>>>>TBookPageWithComponents
#include "QapGenStruct.inl"
//<<<<<+=====TBookPageWithComponents
public:
  void Update(t_context&con)
  {
    Sleep(16);
    /*
      build time after rebuild: 48 sec
      build time after small fix: 18.63 sec

      test2:
      build time after rebuild: Прошло времени 00:00:46.86
      build time after small fix: Прошло времени 00:00:18.38
    */
  }
  void Render(t_context&con)
  {
    //con.context.root.ufli.
  }
};
...
namespace
{
  struct ThisCompileUnit:public ICompileUnit
  {
    ...
    virtual void RegAll(IEnvRTTI&Env)
    {
      ...
      #define LIST(F)\
        F(TType)F(TBookPageWithComponents)F(TBookPageWithScriptList)F(T_2018_BookPage)
      //===+>>>LIST
      LIST(F);
      //<<<+===LIST
      #undef LIST
