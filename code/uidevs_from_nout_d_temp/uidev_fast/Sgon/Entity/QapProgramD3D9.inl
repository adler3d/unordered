...
public:
  ...
  public:
    ...
    #define DEF_PRO_VARIABLE(ADDBEG,ADDVAR,ADDEND)\
    ADDBEG()\
    ADDVAR(TWeakPtr<OwnerProgram>,Win,DEF,$,$)\
    ADDEND()
    //=====+>>>>>DoClose
    #include "QapGenStruct.inl"
    //<<<<<+=====DoClose
...
public:
  ...
  public:
    ...
    #define DEF_PRO_VARIABLE(ADDBEG,ADDVAR,ADDEND)\
    ADDBEG()\
    ADDVAR(TWeakPtr<OwnerProgram>,App,DEF,$,$)\
...
#define DEF_PRO_VARIABLE(ADDBEG,ADDVAR,ADDEND)\
ADDBEG()\
ADDVAR(TWeakPtr<QapProgramD3D9>,pQapProgD3D9,DEF,$,$)\
ADDEND()
//=====+>>>>>TProgramD3D9
#include "QapGenStruct.inl"
//<<<<<+=====TProgramD3D9
...
public:
  ...
  TYPE BuildUserCode(IEnv&Env)
  {
    ...
    {auto*empty_node=root->AddItem<TProgramNode>(Env);empty_node->Caption="empty";}
    auto*node=root->AddItem<TProgramNode>(Env);node->Caption="with QapRenderProgram";
    {auto*empty_node=root->AddItem<TProgramNode>(Env);empty_node->Caption="empty";}
...
public:
  ...
  #define DEF_PRO_VARIABLE(ADDBEG,ADDVAR,ADDEND)\
    ADDBEG()\
    ADDVAR(TFieldPtr,Dest,DEF,$,$)\
    ADDVAR(TFieldPtr,Source,DEF,$,$)\
  ...
  #define DEF_PRO_VARIABLE(ADDBEG,ADDVAR,ADDEND)\
    ADDBEG()\
    ADDVAR(DevCallBack,OnControl,DEF,$,$)\
    ADDVAR(DevCallBack,OnLControl,DEF,$,$)\
    ADDVAR(DevCallBack,OnRControl,DEF,$,$)\
    ADDVAR(KeyVisorProgram,KeyControl,DEF,$,$)\
    ADDVAR(KeyVisorProgram,KeyLControl,DEF,$,$)\
    ADDVAR(KeyVisorProgram,KeyRControl,DEF,$,$)\
    ADDEND()
  //=====+>>>>>DevTools
  #include "QapGenStruct.inl"
  //<<<<<+=====DevTools
...
ADDVAR(QapTextSelectorV4OnDown,OnDownSelect,DEF,$,$)\
ADDVAR(TFieldPtr,Select,DEF,$,$)\
...
public:
  vec2d get_pos()
  {
    if(!Program){QapDebugMsg("WTF?");return vec2d_zero;}
    auto size=Program->WindowGetClientSize();
    return pos+vec2d(-size.x,zD*2.0+size.y)*0.5;
  }
  void Render(QapDev&qDev)
  {
    ...
    QapDev::BatchScope Scope(qDev);
    {
      ...
      for(int i=0;i<lines.size();i++)
      {
        ...
        bool flag=false;
        int h=-dy-2;
        int x=get_pos().x-dx+dx*ex.indent;
      ...
      Clock.Stop();
      QapQ3Text::DrawQapText(&qDev,font,get_pos().x-dx,y,"^1.^0=====[render:"+FToS(Clock.MS())+"]=====^1.");
    }
  }
public:
  ...
  public:
    ...
    void OnValue(OutDevice&OD,LineField&Field)
    {
      ...
      auto*pKB=pTools->KeyControl.Keyboard.get();
      auto&Down=pKB->Down;
      if(!pTools->IsOnDown()&&!Down[VK_DELETE]&&!Down[VK_DIVIDE])return;
      if("bool"==*Field.pType)
      {
...
public:
  void OnBuild(TBuildContext&Context)
  {
    ...
    auto&UserCode=pD3D9Prog->UserCode;UserCode.Caption="UserCode";
    TProgramNode::TArray NodeArr=std::move(UserCode.Programs);
    //BT
    auto*pFontObj=Qrp.ResList.AddItem<QapFontObject>(Env);
    auto*pNode=Qrp.Scene.AddItem<TRenderNodeEx>(Env);pNode->Caption="with QapTreeDebugger";
