...
#ifdef _WIN32
#pragma comment (lib, "QapMicro/QapHumus/libjpeg.lib")
#endif // _WIN32
#endif // NO_JPEG
...
#include "png.h"
#ifdef _WIN32
#pragma comment (lib, "QapMicro/QapHumus/libpng.lib")
#pragma comment (lib, "QapMicro/QapHumus/zlib.lib")
#endif // _WIN32
...
bool Image::unpackImage(){
	...
	ubyte *newPixels;
  if (format == FORMAT_RGBE8){} else if (format == FORMAT_RGB565){
		format = FORMAT_RGB8;
...
bool Image::convert(const FORMAT newFormat){
	...
	if (format == FORMAT_RGBE8 && (newFormat == FORMAT_RGB32F || newFormat == FORMAT_RGBA32F)){} else {
			...
			do {
				...
				if (isFloatFormat(newFormat)){
					if (newFormat <= FORMAT_RGBA32F){
						if (newFormat <= FORMAT_RGBA16F){} else {
							for (int i = 0; i < nDestChannels; i++)	((float *) dest)[i] = rgba[i];
						}
					} else {
						if (newFormat == FORMAT_RGBE8){} else {}
					}
        } else if (newFormat >= FORMAT_I16 && newFormat <= FORMAT_RGBA16){} else if (/*isPackedFormat(newFormat)*/newFormat == FORMAT_RGB10A2){} else {}
