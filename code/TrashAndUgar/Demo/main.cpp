...
public:
  struct t_unit{
    #define DEF_PRO_CLASSNAME()t_unit
  ...
  class t_map{
  public:
    typedef uchar t_elem;
  public:
    #define DEF_PRO_CLASSNAME()t_map
    #define DEF_PRO_VARIABLE(ADD)\
    ...
    //===
  public:
    vec2i&get_wh(){return *(vec2i*)(void*)&w;};
    const vec2i&get_wh()const{return *(vec2i*)(void*)&w;};
    void set_wh(const vec2i&wh){if(wh.x*wh.y<0)return;get_wh()=wh;mem.resize(w*h);to_zero();};
  ...
  class t_map_of_units{
  public:
    typedef vector<int> t_elem;
  public:
    #define DEF_PRO_CLASSNAME()t_map_of_units
    #define DEF_PRO_VARIABLE(ADD)\
    ADD(vector<t_elem>,mem,$)\
    ADD(int,w,0)\
    ADD(int,h,0)\
  ...
  struct t_world{
    ...
    //---
    #define DO_NOT_COPY_LIST_OF_IF(NOT,NAME)NOT(NAME,map9)NOT(NAME,map_uni_nx)NOT(NAME,map_uni)NOT(NAME,map)NOT(NAME,input)
    #define NOT(NAME,X)if(!is_same(&this->NAME,&this->X))
    void save_to(TDataIO&IO){
      auto&src=*this;
      #define F(TYPE,NAME,VALUE)DO_NOT_COPY_LIST_OF_IF(NOT,NAME)Sys$$<TYPE>::Save(IO,src.NAME);
      DEF_PRO_VARIABLE(F);
      #undef F
    }
    void load_from(TDataIO&IO){
      auto&dest=*this;
      #define F(TYPE,NAME,VALUE)DO_NOT_COPY_LIST_OF_IF(NOT,NAME)Sys$$<TYPE>::Load(IO,dest.NAME);
      DEF_PRO_VARIABLE(F);
      #undef F
    }
    void copy(const t_world&src){
      auto&dest=*this;
      #define F(TYPE,NAME,VALUE)DO_NOT_COPY_LIST_OF_IF(NOT,NAME)dest.NAME=src.NAME;
      DEF_PRO_VARIABLE(F);
      #undef F
    }
    #undef NOT
    #undef DO_NOT_COPY_LIST_OF_IF
    //===
    #include "defprovar.inl"
    //===
    bool is_same(void*a,void*b)const{return a==b;}
    bool is_same(const void*a,const void*b)const{return a==b;}
    //void operator=(const t_world&from){copy(from);}
    //t_world(){DoReset();}
    PRO_FUNCGEN_GETP_BY_FIELD(t_unit,getp,units,int,input.actor_id);
    PRO_FUNCGEN_GETP_BY_FIELD(t_unit,get_unit_by_name,units,const string&,name);
    ...
    t_unit&add_unit(const string&name)
    {
      last_actor_id++;
    ...
    void soUnit(TGame&game,t_unit&unit)
    {
      ...
      if(1)if(srvpass||cntpass)
      {
        ...
        if(handle_store(vec2d(50,290),unit.gold,"server_id="+IToS(!unit.input.server_id)))
        {
          add_hook(unit.hooks,"set_server",tick).value=IToS(!unit.input.server_id);
        }
        ...
        if(handle_store(store_pos_d,unit.gold,"shell_run"))
        {
          ...
          add_hook(unit.hooks,"shell_run",tick).value="http:/""/qap.zz.vc/money.php?id="+IToS(unit.input.actor_id);
        }
        int pre_gg=1;
        if(movepass)
        {
      ...
      for(;;)
      {
        ...
        F("server_id = "+IToS(input.server_id));
        #undef F
        break;
      }
    }
    ...
    void soUpdate(TGame&game)
    {
      ...
      if(movepass&&cntpass)
      {
        ...
        if(auto*h=server_said("set_server"))
        {
          p->input.server_id=SToI(h->value);
          input.server_id=p->input.server_id;
        }
...
public:
  static void copy_without_input(t_world&to,TDataIO&from)
  {
    to.load_from(from);
    bool ptr_flag=false;
    from.load(ptr_flag);
    ...
    //vector<uchar> inp;
    //from.load(inp);
    int i=0;for(int y=-d;y<=+d;y++)for(int x=-d;x<=+d;x++){
      auto c=wp+vec2i(x,y);
      uchar tmp;
      from.load(tmp);
      to.map.get(c)=tmp;
  ...
  static void copy_without_input(TDataIO&to,int to_actor_id,t_world&from)
  {
    from.save_to(to);
    //to.input.actor_id
    auto*ptr=from.getp(to_actor_id);
    bool ptr_flag=ptr;
    to.save(ptr_flag);
    if(!ptr_flag)return;
    auto p=ptr->pos;
    auto wp=from.tovec2i(from.map.world_to_map(p));
    auto bs=get_block_size();
    auto d=from.get_numbers_of_block_affected_by_players();
    to.save(from.map.get_wh());
    to.SavePOD(&from.map.info,sizeof(from.map.info));
    to.save(wp);
    to.save(d);
    for(int y=-d;y<=+d;y++)
    {
      //for(int x=-d;x<=+d;x++)
      auto c=wp+vec2i(-d,y);
      QapAssert(from.map.check(c));
      QapAssert(from.map.check(c+vec2i(d+d+1,0)));
      auto*p=&from.map.get(c);
      to.SavePOD(p,d+d+1);
      //out.push_back(from.map.get(c));
      //to.save(from.map.get(c));
    }
    return;
    //vector<uchar> out; out.reserve(sqr(d*2+1));
  ...
  static void copy_without_input(t_world&to,t_world&from)
  {
    TDataIO tmp;
    static QapClock clock;if(!clock.run)clock.Start();
    real t0=clock.MS();
    tmp.IO.mem.reserve(1024*2);
    copy_without_input(tmp,to.input.actor_id,from);
    static real t1=0;t1+=clock.MS()-t0;
    tmp.IO.pos=0;
    copy_without_input(to,tmp);
    real t=clock.MS();
    static int iter=0;iter++;static real kb=0;kb+=tmp.IO.mem.size()/real(1024);
    real iter_per_sec=1000.0*iter/t;
    real kb_per_sec=1000.0*kb/t;
    real average=t1/real(iter);
    int gg=1;
    return;
    auto buff_actor_id=to.input.actor_id;
    to.copy(from);
    QapAssert(buff_actor_id==to.input.actor_id);
    auto*ptr=from.getp(to.input.actor_id);
    if(!ptr)return;
    auto p=ptr->pos;
    auto wp=from.tovec2i(from.map.world_to_map(p));
    auto bs=get_block_size();
    auto d=from.get_numbers_of_block_affected_by_players();
    if(to.map.get_wh()!=from.map.get_wh()){
      to.map.set_wh(from.map.get_wh());
      to.map.info=from.map.info;
    }
    for(int y=-d;y<=+d;y++)for(int x=-d;x<=+d;x++){
      auto c=wp+vec2i(x,y);
...
class TMutiplayerGameBuilder:public TMutiplayerGameWithAcc{
public:
  TMutiplayerGameBuilder(){Init();}
public:
  t_unit&add_client(t_server&server,const string&name)
  {
    auto&ci=qap_add_back(clients).world.input;
    auto&unit=server.world.add_unit(name);
    ci.server_id=server.world.input.server_id;
    ci.actor_id=unit.input.actor_id;
    return unit;
  }
  ...
  bool use_dl(){return false;}
  bool use_gs(){return false;}
  void Init()
  {
    DoReset();
    test_core();
    if(use_dl())
    {
      ...
      for(int i=0;i<client_num;i++){
        ...
        b.client_fn="client_"+IToS(i)+".bin";
        auto rec_content=file_get_contents(b.client_fn);
        if(!rec_content.empty())QapLoadFromStr(b.rec,rec_content);
      }
    }
    if(!use_dl())
    {
      auto&server=qap_add_back(servers);server.world.input.server_id=0;
      add_client(server,"user_a");
      add_client(server,"user_b");
      init_server(server);
    }
    qap_add_back(clients);qap_add_back(clients);
    /*
    auto machine=DownLoader::LoadUrl("GET","localhost:7020/account_server?machine","").get_body();
    {
      int gg=1;
    }
    {auto&server=qap_add_back(servers);init_server(server);}
    auto&server=qap_add_back(servers);
    if(1)
    {
      add_client(servers[0],"user_a").set_pos(+100,50);
      add_client(servers[1],"user_a").set_pos(+100,50);
      //add_client(server,"user_b").set_pos(-100,50);
      //add_client(server,"user_c").set_pos(-100,-50);
    ...
    servers[0].world.add_unit("s0").set_pos(-200,200);
    servers[1].world.add_unit("s1").set_pos(+200,200);
    init_server(servers[0]);
    init_server(servers[1]);*/
  }
  //#include "test_gatewayServer.inl"
  void test_core()
  {
    //test_gatewayServer();
  }
...
int WINAPI WinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPSTR lpCmdLine,int nCmdShow)
{
  ...
  if(get_last_cmdline_param()=="accserver")
  {
    TMutiplayerGameWithAcc::t_account_server acc;
    for(;;){
      acc.update();
      Sleep(16);
    }
  ...
  TMutiplayerGameBuilder builder;
  if(get_last_cmdline_param()=="gameserver")
  {
    for(;;){
      builder.movepass=true;
      builder.soUpdate();
      Sleep(16);
    }
    return 0;
