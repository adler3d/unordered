...
public:
  struct t_cooldown{
    ...
    bool operator()(int tick,int dex){if(tick-t<real(cooldown)/(1.0+(real(dex)-10.0)/35))return false;t=tick;return true;}
  ...
  struct t_map{
    ...
    static int rnd(){
      ...
      static std::uniform_int_distribution<int> distribution(0x0000,0xffff);
  ...
  struct t_world{
    ...
    void copy(const t_world&from){
      ...
      #define F(TYPE,NAME,VALUE)NOT(NAME,map_uni)NOT(NAME,map_mon)NOT(NAME,map_acc)NOT(NAME,map)dest.NAME=from.NAME;
      DEF_PRO_VARIABLE(F);
      #undef NOT
      #undef F
    }
    ...
    void soUnit(TGame&game,t_unit&unit)
    {
      ...
      if(srvpass&&movepass)
      {
        ...
        if(is_bot)
        {
          if(unit.tick%16>=15){unit.input.kb.Down['Q']=true;}else{unit.input.kb.Down['Q']=false;}
        }
        if(unit.input&&!unit.deaded)
        {
          if(unit.input.kb.Down(mbLeft)||unit.autofire)if(unit.weapon.cooldown(unit.tick,unit.stats.dex))
          {
            weapon_fire('M',unit.weapon,unit.pos,screen_to_world(unit.input.mpos)-unit.pos,unit.stats.att,unit.stats.spd);
          }
          ...
          unit.pos+=unit.input.kb.get_dir_from_wasd_and_arrows().SetMag(unit.speed*k).Rot(unit.dir);
      ...
      if(drawpass)
      {
        QapAssert(cntpass);QapAssert(this_actor);
        {
          ...
          circle=QapDev::GenGeomCircleSolid(3,5,0);
    ...
    void unreg_all_units()
    {
      ...
      {
        auto&ex=units[i];
        auto m=tovec2i(world_to_map(map,ex.pos));
    ...
    void soUpdate(TGame&game)
    {
      ...
      if(srvpass)
      {
        ...
        for(int i=0;i<bullets_to_units.size();i++)
        {
          ...
          auto mc=tovec2i(world_to_map(map,b.pos));
        ...
        for(int i=0;i<units.size();i++)
        {
          ...
          //  }
          //}
          if(ex.stats.hp<0)ex.deaded=true;
          {
            ...
            {
              auto c=wp+vec2i(x,y);
              if(!map.check(c))continue;
              auto&cur=map_acc.get_unsafe(c.x,c.y);
              if(abs(tick-cur)>1)
              {
                auto mv=map.get_unsafe(c.x,c.y);if(mv==0x50||mv==0x5f)continue;
              ...
              if(cur==0){
                if(tick!=1){if(rand()%64==0)add_rnd_monster(map_to_world(map,c));}
              }else
              if(tick-cur>64*5)
              {
                auto mv=map.get_unsafe(c.x,c.y);
                if(mv!=0x50&&mv!=0x5f)
                {
                  auto t=(tick-cur)/64;
                  auto n=t/60;
      ...
      if(srvpass)
      {
        ...
        for(int j=0;j<monsters.size();j++)
        {
          ...
          if(tick-ma>64)
          {
            auto&storage=map_mon.get(world_to_map(map,ex.pos));if(&storage==&map_mon.fail_id())continue;
            qap_add_back(storage)=ex;ex.deaded=true;
          }
    ...
    void gen_monster_input(t_monster&ex,vector<vec2d>&nm)
    {
      ...
      auto*p_near_unit=get_near_unit(ex,units);//get_near_unit_from_map(ex.pos,ex.dist);
    ...
    {
      QapAssert(map.check(tovec2i(world_to_map(map,pos))));
      auto&b=add_def_monster();
      b.pos=pos;
      auto m=rand()%7;
...
public:
  ...
  TMutiplayerGame(){
    ...
    {
      bot_x[4]='b'+i;
      qap_add_back(clients).world.input.actor_id=server.world.add_unit(bot_x).set_pos(100*(i%10),100*(i/10)).input.actor_id;
      if(i<100){
        auto&b=server.world.units.back();
        //b.dir=vec2d(1,1).Norm();
        b.input.mpos=vec2d(0,100);
        b.input.consize=vec2d(1,1)*10000;
        b.stats.spd+=rand()%16-8;
        auto&kb=b.input.kb;
        kb.Down[VK_UP]=true;
        kb.Down[mbLeft]=true;
      }
