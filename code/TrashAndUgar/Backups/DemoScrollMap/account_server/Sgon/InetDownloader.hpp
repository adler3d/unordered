...
public:
  ...
  DownLoader(const string &webhost, const string &webpath):shost(),active(0),data(shost.ibuffer),webhost(webhost),webpath(webpath){}
  bool start(){
    shost.Init();
  ...
  static string GetContent(const string&data)
  {
    ...
    string result=tag_http.grip(Extractor(data).extract(tag_http));
    while(e){
