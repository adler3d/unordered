...
{
	if(listener){delete listener; listener=NULL;}	// TCP socket
	if(detector){delete detector; detector=NULL;}	// UDP socket
	kick_all();
	runned=0;
}
...
void GatewayServer::update_net()
{
	if(!listener || !listener->connected) return;

 	if(!net_activ){// запрос подключения
		net_activ=new Socket;
		if(net_activ->setaddr(webserver))return;
		net_activ->sock_addr.sin_port = htons(80);
		net_activ->sock=socket(AF_INET,SOCK_STREAM,IPPROTO_TCP);
		if(net_activ->sock == INVALID_SOCKET)return;
			
		if( net_activ->setnoblock(1)) return;
		connect(net_activ->sock, (sockaddr *)&net_activ->sock_addr, sizeof(net_activ->sock_addr)); // пытаемся установить соединение
		if( net_activ->setnoblock(0)) return;
	}else{
		fd_set fds_src;
		FD_ZERO(&fds_src);
		FD_SET(net_activ->sock,&fds_src);
		fd_set fds_r=fds_src, fds_w=fds_src, fds_e=fds_src;
		if(net_activ->connected)FD_ZERO(&fds_w);
		timeval tv={0,0};
		int ret = ::select(net_activ->sock+1,&fds_r,&fds_w,&fds_e,&tv);
		if(ret){ // есть события
			if(FD_ISSET(net_activ->sock,&fds_r)){ // пришли данные || соединение закрыто
				if(net_activ->recv_raw()<1){
					delete net_activ; net_activ=NULL; // соединение закрыто, данные переданы
					update_net();
					return;
				}
			}
			if(FD_ISSET(net_activ->sock,&fds_w)){ // соединение установлено
				net_activ->connected=1;
				string req(
					"GET %s/active.php?sid=%d&passwd=%s&port=%d HTTP/1.1\n"\
					"Host: %s\n"\
					"User-Agent: Gateway\n"\
				...
				string buff; buff.reserve(req.size()+webpatch.size()+webserver.size()+passwd.size()+20);
				char *pbuff = const_cast<char*>(buff.data());
				int size=sprintf(pbuff,req.c_str(),
					webpatch.c_str(),
					sid,
					passwd.c_str(),
					ntohs(listener->sock_addr.sin_port),
					webserver.c_str()
				);
				req=string(pbuff,size);
				if(net_activ->send_raw(req) == SOCKET_ERROR){// ошибка передачи, повтор подключения
					delete net_activ; net_activ=NULL;
					update_net();
				}else{
					shutdown(net_activ->sock,SD_SEND);
				}
			}
			if(FD_ISSET(net_activ->sock,&fds_e)){ // соединение прервано, повтор подключения
				delete net_activ; net_activ=NULL;
				update_net();
				return;
			}
...
void GatewayClient::scan_net_begin()
{
 	if(scanning_net || net_scann)return;

	scan_net_end();		// очистка старых данных
	scanning_lan=1;

	net_scann=new Socket;
	if(net_scann->setaddr(webserver))return;
	net_scann->sock_addr.sin_port = htons(80);
	net_scann->sock=socket(AF_INET,SOCK_STREAM,IPPROTO_TCP);
	if(net_scann->sock == INVALID_SOCKET)return;
	if(net_scann->setnoblock(1))return;
	connect(net_scann->sock, (sockaddr *)&net_scann->sock_addr, sizeof(net_scann->sock_addr)); // пытаемся установить соединение
	if(net_scann->setnoblock(0))return;
	scanning_net=1;
}

bool GatewayClient::scan_net_result()
{
	if( !scanning_net || !net_scann ) return true;

	fd_set fds_src;
	FD_ZERO(&fds_src);
	FD_SET(net_scann->sock,&fds_src);
	fd_set fds_r=fds_src, fds_w=fds_src, fds_e=fds_src;
	if(net_scann->connected)FD_ZERO(&fds_w);
	timeval tv={0,0};
	int ret = ::select(net_scann->sock+1,&fds_r,&fds_w,&fds_e,&tv);
	if(ret){ // есть события
		if(FD_ISSET(net_scann->sock,&fds_r)){ // пришли данные || соединение закрыто
			if(net_scann->recv_raw()<1){ // соединение закрыто, данные переданы
				string table;
				unsigned offs_table=capture_tag(net_scann->ibuffer,table,"<table>","</table>");
				unsigned offs_tr=0;
				if(offs_table != string::npos){
					while(offs_tr != string::npos){
						string tr;
						offs_tr=capture_tag(table,tr,"<tr>","</tr>");
						if(offs_tr == string::npos) continue;
						table = table.substr(offs_tr);
						GCServer *server = new GCServer;
						server->state.net=1;
						server->s.sock_addr.sin_family=AF_INET;
						unsigned offs_td=0;
						for(int i=0;offs_td != string::npos;++i){
							string td;
							offs_td=capture_tag(tr,td,"<td>","</td>");
							if(offs_td == string::npos) continue;
							tr = tr.substr(offs_td);
							switch(i){
								case 0:break;																//id
								case 1:server->sid=(unsigned)atoi(td.c_str());break;									//sid
								case 2:server->s.sock_addr.sin_addr.s_addr=inet_addr(td.c_str());break;	//ip
								case 3:server->s.sock_addr.sin_port=ntohs(atoi(td.c_str()));break;			//port
							}
						}
						for(int i=0; i<servers.size(); i++)
							if(servers[i]->sid == server->sid){ // обновляем информацию о сервере
								servers[i]->state.net=1;
								delete server; server=NULL;
								break;
							}
							if(server)servers.push_back(server);
				...
				delete net_scann; net_scann=NULL;
				scanning_net=0;
				return true;
			}
		}
		if(FD_ISSET(net_scann->sock,&fds_w)){ // соединение установлено
			net_scann->connected=1;
			string req(
				"GET %s/servers.php?passwd=%s HTTP/1.1\n"\
				"Host: %s\n"\
				"User-Agent: Gateway\n"\
				"Accept: text/html;q=0.9,*/*;q=0.8\n"\
				"Accept-Language: ru-ru,ru;q=0.8,en-us;q=0.5,en;q=0.3\n"\
			...
			if(net_scann->send_raw(req) == SOCKET_ERROR){// ошибка передачи, повтор подключения
				delete net_scann; net_scann=NULL;
				scanning_net=0;
				return true;
			}else{
				shutdown(net_scann->sock,SD_SEND);
			}
		}
		if(FD_ISSET(net_scann->sock,&fds_e)){ // соединение прервано, повтор подключения
			delete net_scann; net_scann=NULL;
			scanning_net=0;
			return true;
		}
...
void GatewayClient::scan_net_end()
{	
	scanning_net=0;
	if(net_scann){delete net_scann;net_scann=NULL;}
}
...
int Socket::setaddr(const std::string &addr){
	...
	if(getaddrinfo(addr.c_str(),NULL,&net_hint,&net_result)==SOCKET_ERROR)return -1;
