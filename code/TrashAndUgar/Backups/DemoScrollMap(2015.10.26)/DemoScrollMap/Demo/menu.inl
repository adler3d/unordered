...
void soListOfPlayers(TGame&game,t_unit&unit,int pix_from_right_border=95)
{
  ...
  const int limit=5;
  vector<t_id_with_dist> arr;arr.reserve(limit);
  t_id_with_dist*pback=nullptr;
  for(int i=0;i<units.size();i++)
  {
    if(&units[i]==&unit)continue;
    auto&ex=units[i];
    t_id_with_dist rec={i,(userpos-ex.pos).SqrMag()};
    if(arr.size()<limit)
    {
      qap_add_back(arr)=rec;
      if(arr.size()==limit){move_max_by_dist_to_back(arr);pback=&arr.back();}
      continue;
    }
    if(pback->dist<=rec.dist)continue;
    *pback=rec;
    move_max_by_dist_to_back(arr);
  }
  sort_by_dist(arr);
  arr.resize(std::min<int>(arr.size(),limit));
  for(;;)
  {
