  ...
  struct t_world{
    ...
    void soUnit(TGame&game,t_unit&unit)
    {
      ...
      if(1)if(srvpass||cntpass)
      {
        ...
        //store
        {
          vec2d store_pos=vec2d(0,0);
          if(drawpass)
          {
            qDev.color=0xffFFFF00;
            qDev.DrawCircleEx(store_pos,20,0,16,0);
          }
          if(unit.pos.dist_to_point_less_that_r(store_pos,unit.r))
          {
            if(unit.gold)
            {
              if(drawpass)
              ...
              if(srvpass)unit.gold--;
            }
          }
        }
        //store b
        {
          vec2d store_pos=vec2d(-100,0);
          if(drawpass)
          {
            qDev.color=0xffFFFF00;
            qDev.DrawCircleEx(store_pos,20,0,16,0);
          ...
          if(unit.pos.dist_to_point_less_that_r(store_pos,unit.r)&&unit.input.kb.OnUp(VK_SHIFT))
          {
            if(unit.gold)
            {
              ...
              if(srvpass)
              {
                //unit.gold=0;
                auto name="do_pause";
                auto*p=get_hook(unit.hooks,name);
                if(!p){p=&qap_add_back(unit.hooks);p->name=name;}
                p->tick=tick;
              }
            }
          }
        ...
        if(movepass)
        {
          ...
          //if(unit.run_this_crap>=0)if(unit.input.run_this_crap>=0){
          int gg=1;
          //unit.run_this_crap=-1;
        }
        int gg=1;
    ...
    void soUpdate(TGame&game)
    {
      ...
      if(movepass&&cntpass)
      {
        ...
        auto server_said=[&p,this](char*cmd)->bool
        {
          if(!p)return false;
          auto*h=get_hook(p->hooks,cmd);if(!h)return false;
        ...
        //{
        //  input.run_this_crap=p->run_this_crap;
        //  auto hwnd=game.win.Form.WinPair.hWnd;
        //  ShowWindow(hwnd,SW_MINIMIZE);
        //  WinApiTool::shell_url("notepad");
        //  //WinApiTool::shell_url("http:/""/ya.ru");
        //  ShowWindow(hwnd,SW_RESTORE);
        //  int gg=1;
        //  //string app="pause";//"C:\\Windows\\System32\\cmd.exe";
        //  //auto hwnd=game.win.Form.WinPair.hWnd;
        //  //{ShowWindow(hwnd,SW_MINIMIZE);system(app.c_str());/*WinApiTool::exec(app,"");*/ShowWindow(hwnd,SW_RESTORE);}
        //}
        //-----
        if(p)
        {
    ...
    /*void get_units_from_map_around_point(const vec2d&pos,real dist,vector<int>&out)
    {
      vector<char> filter;
      auto mc=tovec2i(map_uni_nx.world_to_map(pos));
      if(!map_uni_nx.check(mc))return;
      auto n=1;//+dist/real(map_uni_nx.block_size);
      int c=0;
      for(int y=-n;y<=+n;y++)for(int x=-n;x<=+n;x++)
      {
        auto p=mc+vec2i(x,y);c++;
        if(!map_uni_nx.check(p))continue;
        auto&a=map_uni_nx.get_unsafe(p.x,p.y);
        for(int i=0;i<a.size();i++){
          if(filter.empty())filter.resize(units.size());
          auto&f=filter[a[i]];
          if(f)continue;
          f++;
          out.push_back(a[i]);
    ...
    t_unit*get_near_unit_from_map(const vec2d&pos,real dist)
    {
      ...
      get_units_from_map_around_point(pos,dist,arr);
    ...
    void gen_monster_input(t_monster&ex,vector<vec2d>&nm)
    {
      ...
      if(!p_near_unit){/*ex.dir=vec2d_zero;ex.tick=0;*/return;}
      auto&u=*p_near_unit;
      if((u.pos-ex.pos).Mag()>ex.dist){/*ex.dir=vec2d_zero;ex.tick=0;*/return;}
