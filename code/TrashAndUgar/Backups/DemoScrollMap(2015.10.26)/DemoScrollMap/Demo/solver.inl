/*static vec2d world_to_map(const t_map&map,const vec2d&pos){
  static auto bs=get_block_size();
  static real inv_block_size=1.0/real(bs);
  static auto hbsv=vec2d(1,1)*0.5*bs;
  static auto hbs=bs*0.5;
  vec2d map_offset=vec2d(map.w,map.h)*hbs+hbsv;
  return (map_offset+pos)*inv_block_size;
}*/
/*
...
static t_col_solver_result solve_col_unit_vs_map(t_map&map,t_map&map9,vec2d&unit_pos,int r,int allow_5F)
{
  t_col_solver_result out={0,0,0,vec2i(0,0)};
  auto&cd=out.n;auto&cell_on_map=out.p;
  real block_size=get_block_size();
  auto mp=tovec2i(map.world_to_map(unit_pos));
  if(map9.get(mp)==0)return out;
  ...
  auto m2w=[&](const vec2d&p){return map.map_to_world(vec2d(int(p.x),int(p.y)));};
