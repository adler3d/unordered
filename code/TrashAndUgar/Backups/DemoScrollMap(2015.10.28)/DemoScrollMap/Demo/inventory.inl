...
int get_item_id_under_point(const vec2d&pos,real r)
{
  auto&arr=bags;
  for(int i=0;i<arr.size();i++)
  {
    auto&ex=arr[i];
    if(!ex.pos.dist_to_point_less_that_r(pos,r))continue;
    return i;
  }
  return -1;
}
t_item*get_item(t_world&world,t_unit&unit,t_dd_point&p)
{
  ...
  auto id=get_item_id_under_point(unit.pos,r*3);
      ...
      if(point_in_quad(q,mpos))
      {
        ...
        if(!need_draw)for(;;)
        {
          if(!trade)
          {
            ...
            {
              status=arr[i].name;
              if(arr[i].name=="DEF"){unit.stats.def++;arr[i].deaded=true;}
              if(arr[i].name=="SPD"){unit.stats.spd++;arr[i].deaded=true;}
              if(arr[i].name=="DEX"){unit.stats.dex++;arr[i].deaded=true;}
              if(arr[i].name=="ATT"){unit.stats.att++;arr[i].deaded=true;}
              if(arr[i].name=="CAS"){map.draw_mini_castle(unit.pos);map9.sync(map);/*arr[i].deaded=true;*/}
              break;
            }
