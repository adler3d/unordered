...
auto get_vec3d_color=[&](const vec3f&quad_center,int x,int y,t_pix_info*pinfo)->vec3d
{
  ...
  for(;;)
  {
    ...
    {
      if(two_bounce)QapDebugMsg("no way.");
      ...
      {for(int k=0;k<td.size();k++){
        ...
        if(two_bounce)
        {
          ...
          if(tris_id>=0)
          {
            ...
            {for(int k=0;k<td.size();k++){
              auto dir_id=td[k];
              auto LRC=get_lights_rays_color_in(end_pos,dir_id);
              tris_frag.add(LRC);
          ...
          auto LRC=get_lights_rays_color_in(out.pos,dir_id);frag.add(LRC);
          //frag.add(get_LC_n(out.pos,dir_id));
        }
        #endif
      }}
