      ...
      if(inc_ld_counter)
      {
        ...
        QAP_FOREACH(b.pos2info,{
          ...
          QAP_FOREACH(arr,{
            ...
            auto&t=env.tris[ex];
            t.light_dirs_bits.atomic_set(dir_id);
  ...
  {
    QAP_FOREACH(env.tris,auto&c=ex.light_dirs_bits;c.clear();c.resize(env.dirs.size()));
  ...
  struct StackClockDebugMsg{
    QapClock clock;
    ~StackClockDebugMsg(){QapDebugMsg("time = "+FToS(clock.MS()));}
  };
  bool try_load_env(const string&fn)
  {
    if(access(fn.c_str(),0)!=0){
      ...
      //{
      //  StackClockDebugMsg clock;
      //  QapSaveToFile("savetest_"+fn,tmp);
      //}
      env=std::move(tmp);
      QAP_FOREACH(env.tris,ex.light_dirs_bits.clear());
      //env.dir_id_to_Ltris_list.clear();
      //env.dir_id_to_Dtris_list.clear();
      //QapSaveToFile("withoutLD_and_light_dirs_bits_"+fn,env);
      return true;
    }
  }
  void make_raymaps(const string&fn,int limit)
  {
    //return;
    QapClock clock;
    ...
    QAP_FOREACH(env.tris,{
      ...
      //vector<string> out;
      //int n=0;QAP_FOREACH(t.light_dirs_counter,if(ex)n++;qap_add_back(out)=IToS(ex););
      //file_put_contents("dbg_out.txt",join(out,"\n"));
      //vector<string> tmp;QAP_FOREACH(t.dirs,qap_add_back(tmp)=IToS(ex););
      //file_put_contents("dbg_dir.txt",join(tmp,"\n"));
      //QapDebugMsg("n = "+IToS(n)+"\nt.dirs.size() = "+IToS(t.dirs.size()));
      t.light_dirs_bits.for_each([&](int i,bool ex){if(ex)qap_add_back(t.light_dirs)=i;});
      //QAP_FOREACH(t.light_dirs_counter,{
      //  if(ex>0){
      //    qap_add_back(t.light_dirs)=i;
      //  }
      //});
      //int g=0;QAP_FOREACH(t.light_dirs_counter,if(ex&&qap_find_val(t.dirs,i).size())g++);
      //QapDebugMsg("g = "+IToS(g));
    });
    //QAP_FOREACH(env.tris,auto&t=ex;QAP_FOREACH(t.light_dirs_counter,if(ex>0)qap_add_back(t.light_dirs)=i;));
    
    msg="full done with limit = "+IToS(limit)+"     \n"+FToS(clock.MS())+"ms";
    //QapDebugMsg("before save\n"+msg);
    env.dir_id_to_Ltris_list.clear();
    env.dir_id_to_Dtris_list.clear();
    {
      QapClock clock;
      QapSaveToFile(fn,env);
      auto ms=clock.MS();auto size=get_file_size(fn);
      auto clock_fn="QapSaveToFile.time_in_ms.txt";
      auto con=(file_get_contents(clock_fn)+"\n["+cur_date_str()+"]\n"
        "ms = "+FToS(ms)+"\n"
        "size = "+IToS(size)+"\n"
        "size/ms = "+FToS(size/ms)+"\n\n\n");
      file_put_contents(clock_fn,con);
    }
    //QapDebugMsg("build_rayenv_end\n"+msg);
  }
  static int get_file_size(const string&fn)
  {
    using namespace std;
    fstream f;
