...
DEF_PRO_FIELDS_AND_DORESET()
  ...
  #define DEF_PRO_CTORS_AND_OPERS()\
    ...
    DEF_PRO_CLASSNAME()(SelfClass&&ref){DoReset();oper_set(std::move(ref));}\
    DEF_PRO_CLASSNAME()(){DoReset();}\
    DEF_PRO_CLASSNAME()(const SelfClass&)=default;\
    SelfClass&operator=(const SelfClass&ref)=default;\
  ...
  DEF_PRO_CTORS_AND_OPERS()
    ...
    #define DEF_PRO_ENTRY_AS_PROXYSYS_SAVE(TYPE,NAME,VALUE)QapSave(IO,ref.NAME);
    #define DEF_PRO_ENTRY_AS_PROXYSYS_LOAD(TYPE,NAME,VALUE)QapLoad(IO,ref.NAME);
