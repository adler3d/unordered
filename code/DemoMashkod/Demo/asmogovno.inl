{
  ...
  {proc("func_GenGeomCatmullRom(int,int,int)"){
    PARAM_RESULT("t_out_dev");
  ...
  {proc("func_cr_test(t_input)"){
    ...
    }
    {
      ...
      VAR("int",tmp104);
      {
        ...
        push(var_mpos["y"]);
        push(var_zoom);
        call("func_GenGeomCatmullRom(int,int,int)");
      }
      {
        CALLER_SCOPE();
        pushlea(tmp104);
    ...
    }
    {
      ...
      mov(edi,var_did);
      mov(edx,var_zoom);
      {LOOP_SCOPE();inc(edi);mov(eax,ptr[edi]);mul(eax,edx);stosd_inc();mov(eax,ptr[edi]);mul(eax,edx);stosd_inc();}
    }
    VAR("t_mem",var_mem);
    {
      lea(ecx,var_mem);
