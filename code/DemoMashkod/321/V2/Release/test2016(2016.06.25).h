{
  ...
  struct vector_qap_vertex{
    ...
    int add_with_offset(vector_qap_vertex VA,vec2i offset)
    {
      ...
      //__asm("  lodsd_inc();add(eax,ebx);stosd_inc();");
      //__asm("  lodsd_inc();add(eax,edx);stosd_inc();");
      __asm("}");
      __asm("mov(ecx,this_ptr);");
      /*
  ...
  struct vector_int{
    ...
    int add_mega_fast(vector_int IA,int vpos){
      int sp=IA.p-IA.n;
      int c=IA.n;
      __asm("mov(edi,field_p);mov(esi,var_sp);");
      __asm("mov(eax,param_vpos);");
      __asm("mov(ecx,var_c);");
      //__asm("{LOOP_SCOPE();lodsd_inc();add(eax,edx);stosd_inc();}");
      __asm("rep_addsd_inc(eax);");
      __asm("mov(ecx,this_ptr);");
      /*
  ...
  t_mem cr_test(t_input inp)
  {
    ...
    if(1)
    {
      ...
      od.add(gen_grid(add(obj_pos,mpos)));
      if(0)
      {
        auto quad=od.GenGeomQuad(0,0,10,10);
        for(int x=0;x<64;x++)
