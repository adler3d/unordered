...
public:
  bool go_any_str_from_vec(string&ref,const vector<string>&arr){
    return weak_go_any_str_from_vec<0>(ref,arr);
  }
  template<int>
  bool weak_go_any_str_from_vec(string&ref,const vector<string>&arr)
  {
    t_fallback scope(*this,__FUNCTION__);
    bool&ok=scope.ok;
    QapAssert(!arr.empty());
...
struct t_fallback{
  ...
  /*t_log log;*/
  t_fallback(i_dev&dev,const char*const ptr):dev(dev),ok(true),ptr(ptr),pos(-1),err_count(0){
    if(ptr)dev.push(this);
  }
 ~t_fallback(){
    if(ptr)dev.pop(this);get_qap_fallback_counter()++;
  }/*
  string getStatus(){
    return "["+join(log,",")+"]";
  }*/
...
public:
  ...
  t_fallback main;
public:
  t_load_dev(IEnvRTTI&Env,const string&mem,size_t pos=0):Env(Env),mem(mem),pos(pos),maxpos(pos),main(*(i_dev*)nullptr,nullptr){stack.push_back(&main);}
public:
  void push(t_fallback*ptr){
    ...
    stack.push_back(ptr);
    ptr->pos=this->pos;
    ptr->mandatory.mandatory=true;
    ptr->mandatory.scope=ptr;
    ptr->optional.mandatory=false;
