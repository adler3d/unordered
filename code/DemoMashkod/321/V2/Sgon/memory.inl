...
//<<<<<+=====IProduct
public:
  //static void SysHasSelf();ADDVAR(0,TSelfPtr<SelfClass>,Self,DEF,$,$)
public:
  //bool flag_init,flag_free;
  //IProduct()
  //{
  //  flag_init=false;
  //  flag_free=false;
...
#define DEF_PRO_VARIABLE(ADDBEG,ADDVAR,ADDEND)\
ADDBEG()\
ADDVAR(0,TMemory,Memory,DEF,$,$)\
ADDVAR(1,THardPtr<IAllocator>,Alloc,DEF,$,$)\
...
ADDBEG()\
ADDVAR(0,TMemory,Memory,DEF,$,$)\
ADDVAR(1,THardPtr<TType>,pType,DEF,$,$)\
ADDEND()
//=====+>>>>>TRTTIProduct
#include "QapGenStruct.inl"
//<<<<<+=====TRTTIProduct
...
public:
  ...
  #define DEF_PRO_VARIABLE(ADDBEG,ADDVAR,ADDEND)\
  ADDBEG()\
  ADDVAR(0,int,AllocCount,SET,0,$)\
  ADDVAR(1,int,DeallocCount,SET,0,$)\
  ADDVAR(2,int,MemoryUsed,SET,0,$)\
  ADDEND()
  //=====+>>>>>TMemInfo
  #include "QapGenStruct.inl"
  //<<<<<+=====TMemInfo
...
#define DEF_PRO_VARIABLE(ADDBEG,ADDVAR,ADDEND)\
ADDBEG()\
ADDVAR(0,THardPtr<TTypeStruct>,pType,DEF,$,$)\
ADDEND()
//=====+>>>>>TDynStructFactory
#include "QapGenStruct.inl"
//<<<<<+=====TDynStructFactory
...
#define DEF_PRO_VARIABLE(ADDBEG,ADDVAR,ADDEND)\
ADDBEG()\
ADDVAR(0,THardPtr<TTypeSelfPtr>,pType,DEF,$,$)\
ADDEND()
//=====+>>>>>TDynSelfPtrFactory
#include "QapGenStruct.inl"
//<<<<<+=====TDynSelfPtrFactory
