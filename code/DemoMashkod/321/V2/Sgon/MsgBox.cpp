...
ADDVAR(1,int,id,SET,0,$)\
ADDVAR(2,int,hwnd,SET,0,$)\
ADDEND()
//=====+>>>>>TWinButton
...
#define DEF_PRO_VARIABLE(ADDBEG,ADDVAR,ADDEND)\
ADDBEG()\
ADDVAR(0,string,Caption,SET,"TMsgBox",$)\
ADDVAR(1,bool,NeedClose,SET,false,$)\
ADDVAR(2,bool,Runned,SET,false,$)\
ADDVAR(3,bool,Visible,SET,true,$)\
ADDVAR(4,eQuad,Quad,DEF,$,$)\
ADDVAR(5,TForm,Form,SET,this,$)\
ADDVAR(6,int,zDelta,SET,0,$)\
ADDVAR(7,vector<TWinButton>,buttons,DEF,$,$)\
ADDVAR(8,TWinChat,chat,DEF,$,$)\
ADDVAR(9,int,retval,SET,0,$)\
ADDEND()
//=====+>>>>>TMsgBox
#include "QapGenStruct.inl"
//<<<<<+=====TMsgBox
...
class TMsgBoxToolFunc{
public:
#define DEF_PRO_NESTED(F)F(UserMsg)
#define DEF_PRO_STRUCT_INFO(NAME,PARENT,OWNER)NAME(TMsgBoxToolFunc)
#define DEF_PRO_VARIABLE(ADDBEG,ADDVAR,ADDEND)\
ADDBEG()\
ADDEND()
//=====+>>>>>TMsgBoxToolFunc
...
//<<<<<+=====TMsgBoxToolFunc
public:
  class UserMsg:public i_sys_impl{
  #define DEF_PRO_STRUCT_INFO(NAME,PARENT,OWNER)NAME(UserMsg)PARENT(i_sys_impl)OWNER(TMsgBoxToolFunc)
  #define DEF_PRO_VARIABLE(ADDBEG,ADDVAR,ADDEND)\
  ADDBEG()\
  ...
  //<<<<<+=====UserMsg
  public:
    void exec(){UberInfoBox("UserMsg",text);}
  };
};
...
namespace
{
  struct ThisCompileUnit:public ICompileUnit
  {
    ...
    virtual void RegAll(IEnvRTTI&Env)
    {
      ...
      #define LIST(F)\
        F(TType)F(TMsgBox)F(TMsgBoxToolFunc)
      //===+>>>LIST
      LIST(F);
      //<<<+===LIST
      #undef LIST
