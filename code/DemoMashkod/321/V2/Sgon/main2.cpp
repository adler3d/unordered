...
ADDVAR(1,vector<TAutoPtr<IRenderItem>>,Items,DEF,$,$)\
ADDEND()
//=====+>>>>>TMachineRenderItem
...
ADDBEG()\
ADDVAR(0,TSelfPtr<SelfClass>,Self,DEF,$,$)\
ADDVAR(1,TWeakPtr<QapFontObject>,FontObject,DEF,$,$)\
ADDVAR(2,string,Text,DEF,$,$)\
ADDVAR(3,vec2f,pos,SET,vec2f(600,20),$)\
ADDEND()
//=====+>>>>>QapQ3Text_v0
#include "QapGenStruct.inl"
//<<<<<+=====QapQ3Text_v0
...
ADDBEG()\
ADDVAR(0,TSelfPtr<SelfClass>,Self,DEF,$,$)\
ADDVAR(1,TWeakPtr<QapTex>,pTex,DEF,$,$)\
ADDVAR(2,vector<vec2d>,points,DEF,$,$)\
ADDVAR(3,vec2d,size,SET,vec2d(32,32),$)\
ADDVAR(4,real,ang,SET,0.0,$)\
ADDEND()
//=====+>>>>>qap_PointList
#include "QapGenStruct.inl"
//<<<<<+=====qap_PointList
...
ADDVAR(1,string,str_value,DEF,$,$)\
ADDVAR(2,bool,auto_inc,SET,true,$)\
ADDVAR(3,bool,auto_itos,SET,true,$)\
ADDVAR(4,TWeakPtr<IProgramD3D9>,program,DEF,$,$)\
ADDVAR(5,TWeakPtr<qap_PointList>,points,DEF,$,$)\
ADDEND()
//=====+>>>>>MeUnit1
#include "QapGenStruct.inl"
//<<<<<+=====MeUnit1
...
#define DEF_PRO_VARIABLE(ADDBEG,ADDVAR,ADDEND)\
ADDBEG()\
ADDVAR(0,int,id,SET,0,$)\
ADDVAR(1,bool,need_init,SET,false,$)\
ADDVAR(2,bool,need_free,SET,false,$)\
ADDVAR(3,bool,need_inc,SET,false,$)\
ADDVAR(4,bool,need_dec,SET,false,$)\
ADDVAR(5,string,message,SET,"noname",$)\
ADDVAR(6,vector<string>,arr,DEF,$,$)\
ADDVAR(7,bool,auto_update,SET,true,$)\
ADDEND()
//=====+>>>>>MeUnit2
#include "QapGenStruct.inl"
//<<<<<+=====MeUnit2
...
ADDBEG()\
ADDVAR(0,TSelfPtr<SelfClass>,Self,DEF,$,$)\
ADDVAR(1,int,buffer,SET,0,$)\
ADDVAR(2,bool,need_init,SET,false,$)\
ADDVAR(3,TFieldPtr,ptr,DEF,$,$)\
ADDEND()
//=====+>>>>>MeUnit3
#include "QapGenStruct.inl"
//<<<<<+=====MeUnit3
...
#define DEF_PRO_VARIABLE(ADDBEG,ADDVAR,ADDEND)\
ADDBEG()\
ADDVAR(0,int,SizeA,SET,32,$)\
ADDVAR(1,int,SizeB,SET,16,$)\
ADDEND()
//=====+>>>>>TGUI_MouseTrigon
#include "QapGenStruct.inl"
//<<<<<+=====TGUI_MouseTrigon
...
ADDBEG()\
ADDVAR(0,TSelfPtr<SelfClass>,Self,DEF,$,$)\
ADDVAR(1,TWeakPtr<IUnitProgram>,LinkA,DEF,$,$)\
ADDVAR(2,TWeakPtr<IUnitProgram>,LinkB,DEF,$,$)\
ADDVAR(3,TWeakPtr<IUnitProgram>,LinkC,DEF,$,$)\
ADDVAR(4,TAutoPtr<IUnitProgram>,Other,DEF,$,$)\
ADDEND()
//=====+>>>>>TUnit20121101
#include "QapGenStruct.inl"
//<<<<<+=====TUnit20121101
...
public:
  ...
  public:
    ...
    ADDBEG()\
    ADDVAR(0,TSelfPtr<SelfClass>,Self,DEF,$,$)\
    ADDVAR(1,bool,Passed,SET,false,$)\
    ADDEND()
    //=====+>>>>>TCallback
    #include "QapGenStruct.inl"
    //<<<<<+=====TCallback
...
#define DEF_PRO_VARIABLE(ADDBEG,ADDVAR,ADDEND)\
ADDBEG()\
ADDVAR(0,TWeakPtr<QapTex>,pTex,DEF,$,$)\
ADDVAR(6,KeyVisor,Key,DEF,$,$)\
ADDVAR(7,TCallback,OnDown,DEF,$,$)\
ADDVAR(5,bool,need_init_key,SET,false,$)\
ADDVAR(4,bool,drawpolyline,SET,false,$)\
ADDVAR(2,vec2i,size,SET,vec2i(32,32),$)\
ADDVAR(3,real,ang,SET,0.0,$)\
ADDVAR(1,vector<vec2i>,points,DEF,$,$)\
ADDEND()
//=====+>>>>>TGUI_PointList
#include "QapGenStruct.inl"
//<<<<<+=====TGUI_PointList
