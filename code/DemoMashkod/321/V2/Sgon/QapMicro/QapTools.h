...
struct CharMask
{
  ...
  CharMask(const char(&allowed)[COUNT]){
    string str;
    str.resize(COUNT-1);
...
//-------------------------------------------//
inline bool is_intel_endian()
{
  union{struct{unsigned char a,b;};unsigned short v;} s;
  s.v=0x0100;
  return s.b;
}
