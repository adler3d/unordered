...
public:
  ...
  #define DEF_PRO_VARIABLE(ADDBEG,ADDVAR,ADDEND)\
  ADDBEG()\
  ADDVAR(0,vector<TAutoPtr<i_sep>>,arr,DEF,$,$)\
  ADDEND()
  //=====+>>>>>t_impl
  #include "QapGenStruct.inl"
  //<<<<<+=====t_impl
...
public:
  ...
  public:
    bool go(i_dev&dev){
      ...
      D+=dev.go_any_str_from_vec(
        value,
        g_static_split_arr_0
      );
      if(!ok)return ok;
      return ok;
    }
  ...
  public:
    bool go(i_dev&dev){
      ...
      auto&M=scope.mandatory;
      auto&O=scope.optional;
      static const string dip_for_a=gen_dips("azAZ")+"_$@";
      M+=dev.go_any_char(A,dip_for_a);
      if(!ok)return ok;
      static const string dip_for_b=gen_dips("azAZ09")+"_$@";
...
ADDVAR(1,vector<TAutoPtr<i_expr>>,body,DEF,$,$)\
ADDEND()
//=====+>>>>>t_param
#include "QapGenStruct.inl"
//<<<<<+=====t_param
...
#define DEF_PRO_VARIABLE(ADDBEG,ADDVAR,ADDEND)\
ADDBEG()\
ADDVAR(0,t_name,cmd,DEF,$,$)\
ADDVAR(1,vector<t_param>,params,DEF,$,$)\
ADDEND()
//=====+>>>>>t_line
#include "QapGenStruct.inl"
//<<<<<+=====t_line
