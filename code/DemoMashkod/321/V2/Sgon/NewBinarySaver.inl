...
class MiniMetaSpec{
public:
#define DEF_PRO_STRUCT_INFO(NAME,PARENT,OWNER)NAME(MiniMetaSpec)
#define DEF_PRO_VARIABLE(ADDBEG,ADDVAR,ADDEND)\
ADDBEG()\
...
//<<<<<+=====MiniMetaSpec
public:
  void GrabFrom(TSpaceDependsTools&SPD)
  {
    auto&out=Arr;
    auto&Space=SPD.Space;
    auto&arr=SPD.DeclareTypeArray;
    int count=arr.size();
    ...
    auto copy_count=[&Space,&get](TType*pType)
    {
      auto&item=Space[pType->Self];
      QapAssert(!item.count);
      int count=Space[get(pType)].count;
      if(count==item.count)return;
      item.count=count;
    };
    auto pass=[&Space,&get,&out](TType*pType)
    {
      QapAssert(Space[((DeclareType*)Space[pType->Self].ptr_lnk.get())->Self].id==get(pType));
      auto&item=Space[get(pType)];
      if(!item.count)return;
      out.push_back(pType->GetFullName());
      int back_id=out.size();
      int id=item.id;
      QapAssert(id>=back_id);
      if(id!=back_id)Space.Swap(id,back_id);
    };
    for(int i=0;i<arr.size();i++)
    {
      auto&ex=arr[i];
      copy_count(ex.second);
    }
    for(int i=0;i<arr.size();i++)
    {
      auto&ex=arr[i];
  ...
  bool ApplyTo(TSpaceDependsTools&SPD)
  {
    auto&input=Arr;
    auto&Space=SPD.Space;
    auto&arr=SPD.DeclareTypeArray;
    ...
    auto pass=[&Space,&get,&input](TType*pType,int id,const string&key)->bool
    {
      if(key!=pType->GetFullName())return false;
      Space.Swap(id,get(pType));
      return true;
    };
    auto link_keyword=[&Space,&get,&pass,&arr](int id,const string&key)->bool
    {
      for(int i=0;i<arr.size();i++)
      {
        auto&ex=arr[i];
        if(pass(ex.first,id,key))return true;
        if(pass(ex.second,id,key))return true;
      }
      return false;
    };
...
public:
  typedef TServerPtrSpace TSpace;
public:
  void InitSpaceAdv(TSpace&SpaceAdv,TSpace&Space)
  {
    auto&MetaSpec=*this;
    for(int i=1;i<1+MetaSpec.Arr.size();i++)
    {
      QapAssert(Space[i].ptr.IsValid()&&SpaceAdv[i].ptr.IsNull());
      SpaceAdv.Arr[i].copy(Space[i]);
    }
  }
  void FreeSpaceAdv(TSpace&SpaceAdv)
  {
    auto&MetaSpec=*this;
    for(int i=1;i<1+MetaSpec.Arr.size();i++)
    {
      auto&item=SpaceAdv.Arr[i];
      item.ptr=nullptr;
      item.ptr_lnk=nullptr;
    }
  }
};
*/
...
public:
  typedef TServerPtrSpace TSpace;
  static void DoSave(ISmartPtrSolver&Solver,TQapIO&IO,TType*pType,void*pValue)
  {
