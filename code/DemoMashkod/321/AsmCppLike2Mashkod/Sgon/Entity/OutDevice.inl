...
public:
  vector<TIndentStrLine> lines;
  void AddText(const string&text)
  {
    if(DevContext.eos)
    {
      DevContext.omitted_lines++;
      return;
    }
    LineField tmp;
    {}
...
public:
  TIndentStrLine&add_empty_line()
  {
    lines.push_back(std::move(TIndentStrLine()));
    auto&outline=lines.back();
...
public:
  void AddLineFromField(LineField&line)
  {
    ...
    if(DevContext.InScreenOptimization)
    {
      ...
      bool bos=y<ScreenMaxY;
      bool eos=y<ScreenMinY;
      DevContext.bos=bos;
      DevContext.eos=eos;
      if(!bos)DevContext.empty_lines++;
      if(eos)DevContext.omitted_lines++;
      if(!bos||eos)return;
    }
    //
...
public:
  ...
  {
    dev.AddValue("...TTypeArray is under construction...");
  }
  void Do(TTypeSys*p)
  {
    if(dev.DevContext.eos)
    {
      dev.DevContext.omitted_lines++;
      return;
    }
    if(p->TxtIO)
    {
  ...
  void Do(TTypeVector*p)
  {
    ...
    if(p_elemTypefullname=="TIndentStrLine"){
      int count=VecAPI->GetCount(pValue);
      dev.AddValue("..."+IToS(count)+"...");
      return;
    }
