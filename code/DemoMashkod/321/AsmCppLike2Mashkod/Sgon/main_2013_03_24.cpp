...
ADDVAR(1,string,ip,DEF,$,$)\
ADDVAR(2,int,id,SET,0,$)\
ADDEND()
//=====+>>>>>TWebRecTimeIpId
...
ADDVAR(1,TAutoPtr<TDownLoader>,loader,DEF,$,$)\
ADDVAR(2,string,webhost,DEF,$,$)\
ADDVAR(3,string,webpath,DEF,$,$)\
ADDVAR(4,bool,need_init,SET,true,$)\
ADDVAR(5,bool,enabled,SET,true,$)\
ADDVAR(6,int,tick,SET,0,$)\
ADDVAR(7,int,loader_tick,SET,0,$)\
ADDVAR(10,int,prev_time,SET,0,$)\
ADDVAR(11,int,cur_delta,SET,0,$)\
ADDVAR(12,int,interval,SET,10000,$)\
ADDVAR(20,string,recv_buff,DEF,$,$)\
ADDVAR(30,bool,is_active_loader,SET,false,$)\
ADDEND()
//=====+>>>>>TWebProgram
#include "QapGenStruct.inl"
//<<<<<+=====TWebProgram
...
#define DEF_PRO_VARIABLE(ADDBEG,ADDVAR,ADDEND)\
ADDBEG()\
ADDVAR(0,TWeakPtr<TWebProgram>,webapp,DEF,$,$)\
ADDVAR(1,vector<TWebRecTimeIpId>,recv_buff,DEF,$,$)\
ADDEND()
//=====+>>>>>TWebStatusIndicator
#include "QapGenStruct.inl"
//<<<<<+=====TWebStatusIndicator
...
#define DEF_PRO_VARIABLE(ADDBEG,ADDVAR,ADDEND)\
ADDBEG()\
ADDVAR(0,TEmptyClass,empty,DEF,$,$)\
ADDVAR(1,string,text,DEF,$,$)\
ADDVAR(2,vec2d,pos,SET,vec2d(0,0),$)\
ADDVAR(3,TWeakPtr<TType>,type,DEF,$,$)\
ADDEND()
//=====+>>>>>TDummyClass
#include "QapGenStruct.inl"
//<<<<<+=====TDummyClass
