...
class TPlainSaver:public IVisitorRTTI{
public:
  typedef TPlainSaver SelfClass;
  typedef TPlainCodeGen TSaveDev;
...
public:
  ...
  void*pValue;
  bool hack_flag;
  TPlainSaver(ISmartPtrSolver&Solver,TSaveDev&dev,void*pValue):Solver(Solver),dev(dev),pValue(pValue),hack_flag(false){}
  TPlainSaver(TPlainSaver&owner,void*pValue):Solver(owner.Solver),dev(owner.dev),pValue(pValue),hack_flag(false){}
public:
...
public:
  void Do(TTypeSys*p)
  {
    ...
    func(out,pValue);
    dev.content+=out+";";
    int l=int(dev.content.size());
    size_t pos=dev.content.rfind('\n');
    int d=l-pos;
    if(pos==std::string::npos)d=l;
    if(d>60){
      dev.content+="\n";
    }
  }
...
#define DEF_PRO_VARIABLE(ADDBEG,ADDVAR,ADDEND)\
ADDBEG()\
ADDVAR(0,EnvDefEntryPoint,Decl,DEF,$,$)\
...
#define DEF_PRO_VARIABLE(ADDBEG,ADDVAR,ADDEND)\
ADDBEG()\
ADDVAR(0,EnvDefTypeSymbol,Decl,DEF,$,$)\
ADDVAR(1,EnvDefEntryPoint,Impl,DEF,$,$)\
ADDEND()
//=====+>>>>>EnvDefTypeStr
#include "QapGenStruct.inl"
//<<<<<+=====EnvDefTypeStr
...
#define DEF_PRO_VARIABLE(ADDBEG,ADDVAR,ADDEND)\
ADDBEG()\
ADDVAR(0,TWeakPtr<DeclareType>,Decl,DEF,$,$)\
ADDVAR(1,EnvDefEntryPoint,Impl,DEF,$,$)\
ADDEND()
//=====+>>>>>EnvDefTypePtr
#include "QapGenStruct.inl"
...
#define DEF_PRO_VARIABLE(ADDBEG,ADDVAR,ADDEND)\
ADDBEG()\
ADDVAR(0,DeclareEnvRTTI,DeclEnv,DEF,$,$)\
ADDVAR(1,vector<EnvDefTypePtr>,ImplPtrs,DEF,$,$)\
ADDEND()
//=====+>>>>>FullDeclareEnvRTTI
#include "QapGenStruct.inl"
//<<<<<+=====FullDeclareEnvRTTI
...
#define DEF_PRO_VARIABLE(ADDBEG,ADDVAR,ADDEND)\
ADDBEG()\
ADDVAR(0,int,SpaceSize,SET,0,$)\
ADDVAR(1,vector<EnvDefTypeStr>,MetaSpec,DEF,$,$)\
ADDVAR(2,FullDeclareEnvRTTI,FullDeclEnv,DEF,$,$)\
ADDVAR(3,THardPtr<TType>,RootType,DEF,$,$)\
ADDEND()
//=====+>>>>>TQapBinaryObject
#include "QapGenStruct.inl"
//<<<<<+=====TQapBinaryObject
...
struct TConservationMachine 
{
  ...
  struct TPointsHolder
  {
    ...
    void on_with_hack(TSpace&Space)const
    {
      for(int i=0;i<Arr.size();i++)
      {
        auto&ex=Arr[i];
        if(ex.ptr.IsNull())continue;
    ...
    void off_all_objects_with_hack(TSpace&Space)const
    {
    ...
    void off_with_hack(TSpace&Space)const
    {
      for(int i=0;i<Arr.size();i++)
      {
        auto&ex=Arr[i];
    ...
    void insert_to_end_with_hack(TSpace&Space)const
    {
      for(int i=0;i<Arr.size();i++)
      {
        auto&ex=Arr[i];
  ...
  struct TSyncPointsHolder:TPointsHolder
  {
    ...
    void sync_with_hack(TSpace&Space)
    {
      for(int i=0;i<Arr.size();i++)
      {
        auto&ex=Arr[i];
    ...
    void update_id_with_hack(const TSpace&Space)
    {
      auto&arr=Arr;
      for(int i=0;i<arr.size();i++)
      {
    ...
    void insert_and_sync_with_hack(TSpace&Space,TSpace&SpaceForIt)
    {
      auto&UpA=*this;
      UpA.off_with_hack(Space);
      UpA.insert_to_end_with_hack(SpaceForIt);
      SelfClass UpB;
      UpB.copy(UpA);
      UpB.update_id_with_hack(SpaceForIt);
      UpB.off_all_objects_with_hack(SpaceForIt);
      UpA.on_with_hack(Space);
      UpB.sync_with_hack(Space);
    }
    void insert_and_sync(TSpace&Space,TSpace&SpaceForIt)
    {
      insert_and_sync_with_hack(Space,SpaceForIt);
      return;
      auto&UpA=*this;
      UpA.off(Space);
...
class QapPlainObject{
public:
  typedef TServerPtrSpace TSpace;
  static void DoSave(ISmartPtrSolver&Solver,TPlainCodeGen&OutDev,TType*pType,void*pValue)
  {
    TPlainSaver BS(Solver,OutDev,pValue);
    pType->Use(BS);
  }
...
struct UberSaveDevicePlain:IUberSaveDevice
{
  IQapStream&Stream;
  struct TContext
  {
    TPlainCodeGen&OutDev;
    TSmartPtrSolver&Solver;
  };
  TContext*pContext;
  UberSaveDevicePlain(IQapStream&Stream):Stream(Stream),pContext(nullptr){}
  ...
  void Save(TMachine&machine,TRawPtr&raw_ptr)
  {
    ...
    auto&Content=Stream.GetContent();
    {
      ...
      TPlainCodeGen OutDev={Content.IO.mem};
  ...
  void DoSave(TMachine&machine,TRawPtr&raw_ptr)
  {
    ...
    QapPlainObject::DoSave(Solver,OutDev,raw_ptr.pType,raw_ptr.pValue);
  }
};
...
bool QapPublicUberFullSaverPlain(IEnvRTTI&Env,IQapRawObject&&Object,const string&filename)
{
  return QapUberFullSaver(Env,std::move(Object),UberSaveDevicePlain(TQapFileStream(filename,false)));
}