...
ADDVAR(TWeakPtr<QapFontObject>,Font,DEF,$,$)\
ADDVAR(vec2d,Pos,SET,vec2d(+400,+280),$)\
ADDVAR(vec2d,MPos,SET,vec2d(0,0),$)\
ADDVAR(int,zDelta,SET,0,$)\
ADDVAR(int,SelectId,SET,-1,$)\
ADDVAR(vector<QapIndentLine>,Lines,DEF,$,$)\
ADDEND()
//=====+>>>>>QapTextSelectorV1
#include "QapGenStruct.inl"
//<<<<<+=====QapTextSelectorV1
...
ADDBEG()\
ADDVAR(TSelfPtr<SelfClass>,Self,DEF,$,$)\
ADDVAR(TWeakPtr<KeyVisorProgram>,KeyVisor,DEF,$,$)\
ADDVAR(TWeakPtr<TType>,base,DEF,$,$)\
ADDVAR(TWeakPtr<TType>,select,DEF,$,$)\
ADDVAR(bool,need_refresh,SET,false,$)\
ADDVAR(bool,auto_update,SET,true,$)\
ADDVAR(vector<TWeakPtr<TType>>,list,DEF,$,$)\
ADDVAR(QapTextSelectorV1,selector,DEF,$,$)\
ADDEND()
//=====+>>>>>QapTextSelectorV0
#include "QapGenStruct.inl"
//<<<<<+=====QapTextSelectorV0
...
#define DEF_PRO_VARIABLE(ADDBEG,ADDVAR,ADDEND)\
ADDBEG()\
ADDVAR(TWeakPtr<QapTextSelectorV0>,Selector,DEF,$,$)\
ADDVAR(TWeakPtr<KeyVisorProgram>,KeyVisor,DEF,$,$)\
ADDVAR(TWeakPtr<QapTextSelectorV0OnDown>,OnDown,DEF,$,$)\
ADDEND()
//=====+>>>>>MeUnit4
#include "QapGenStruct.inl"
//<<<<<+=====MeUnit4
