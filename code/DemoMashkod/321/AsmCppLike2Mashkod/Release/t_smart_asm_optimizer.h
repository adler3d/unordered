class t_smart_asm_optimizer:public t_smart_asm{
public:
  struct t_var{
    string name;
    string type;
...
public:
  ...
  public:
    typedef t_smart_asm_optimizer t_dev;
    t_dev&dev;
  ...
  public:
    struct t_calc_params_tool
    {
      ...
      vector<string> types;
      void calc(t_smart_asm_optimizer&dev,t_params&params)
      {
        //calc types
        auto&arr=params.arr;
        for(int i=0;i<arr.size();i++){
      ...
      void calc(t_smart_asm_optimizer&dev,TAutoPtr<t_params>&params){
        if(!params)return;
        auto&ref=*params.get();
        calc(dev,ref);
      }
  ...
  public:
    ...
    {
      QapAssert(!p->out.ext);
      auto&var=unsafe_find_var(p->out);
      var.write_counter++;
      //sys way
      if(true)
      {
...
public:
  struct t_reg_id{
    ...
    t_var&get(t_smart_asm_optimizer&ref)const{
...
public:
  ...
  /*copy-paste*/  public:
  /*copy-paste*/    t_smart_asm_optimizer&machine;
...
public:
  t_smart_asm_optimizer(IEnvRTTI&Env):Env(Env){}
public:
