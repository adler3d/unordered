...
public:
  ...
  //===
public:
  t_cd_dev(t_perf_sys&perf_sys,t_cd_arr&cd_arr,vector<t_point>&points,real r):perf_sys(perf_sys),cd_arr(cd_arr),points(points),r(r)
  {
    DoReset();
...
public:
  ...
  void cd_out()
  {
    t_clock clock(perf_sys,"cd_out");
    {
      t_clock clock(perf_sys,"cd_fix_order");
      auto&arr=cd_arr;
      for(int i=0;i<arr.size();i++)
      {
        auto&ex=arr[i];
    ...
    {
      t_clock clock(perf_sys,"cd_sort");
      if(!cd_arr.empty())
      {
  ...
  void cd_out_fast()
  {
    ...
    int gg=1;
    {
      t_clock clock(perf_sys,"cd_out_gen");
      int id=0;
...
public:
  ...
  t_minmax get_minmax()
  {
    t_minmax out;
    auto&arr=points;
    t_clock clock(perf_sys,"cd_minmax");
    for(int i=0;i<arr.size();i++)
    {
      auto&a=arr[i];
      if(!i)out.minp=a.pos;
...
public:
  ...
  {
    t_clock clock(perf_sys,"cd_solver");
    auto d=r+r;
    auto inv_d=1.0/d;
