...
interface
uses
  Windows,Scene,eXgine,OpenGL,ASDVector,QuickList,GameRender;
const
  FontSize=16;
type
  TMenuItem=class; TNotifyEvent=procedure(Sender:TObject) of object;
  TMenu=class(TScene)
  private
    FFont:TFont;
    FName:string;
...
begin
  ...
  begin
    ...
    tex.Enable(FSelTex); glColor4d(1,1,1,0.2);
    DrawRect((Width-FLastML-Ot)/2,(Height-FontSize*2*ItemsCount)/2+Select*FontSize*2,FLastML+Ot,FontSize*2);
    tex.Disable; glColor4d(1,1,1,1);
    ML:=-1;
